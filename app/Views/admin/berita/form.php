...
<div class="w-full max-w-full">
    <div class="mb-4 flex items-center justify-between">
        <div>
            ...
            <p class="text-xs text-slate-500 dark:text-slate-400">
                Kelola konten berita untuk ditampilkan di website desa.
            </p>
        </div>
    ...
    <div class="rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-900">
        ...
        <div class="px-4 py-4 md:px-5 md:py-5 space-y-3">
            ...
            <form action="<?= base_url('admin/berita/save') ?>" method="post" class="space-y-3" enctype="multipart/form-data">
                <?= csrf_field() ?>
                ...
                <div>
                    ...
                    <input type="text" name="title" id="title"
                        ...
                        placeholder="Misal: Musyawarah Desa Tahun 2025" required>
                </div>

                <div class="grid gap-3 md:grid-cols-[2fr,3fr]">
                    <div class="space-y-2">
                        <label class="block text-xs font-medium text-slate-700 dark:text-slate-200 mb-1" for="cover_image">
                            Gambar Utama / Thumbnail
                        </label>

                        <?php if (!empty($news['cover_image'])): ?>
                            <div class="mb-2">
                                <div class="w-full max-w-[220px] aspect-[4/3] rounded-xl overflow-hidden border border-slate-200 bg-slate-50 dark:border-slate-700 dark:bg-slate-800">
                                    <img src="<?= base_url('file/news/' . $news['cover_image']) ?>"
                                        alt="cover"
                        ...
                        <p class="mt-1 text-[11px] text-slate-500 dark:text-slate-400">
                            Disarankan rasio 4:3 atau 16:9, ukuran maksimal &lt; 3 MB. Jika kosong, berita tetap tersimpan tanpa gambar utama.
                        </p>
                    </div>
                ...
                <div>
                    ...
                    <textarea name="content" id="content" rows="12"
                        class="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs md:text-sm text-slate-800 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100"><?= esc($news['content'] ?? '') ?></textarea>
                </div>
...
<script>
    (function() {
        ...
        // pakai endpoint upload image yang sudah ada di Halaman Statis
