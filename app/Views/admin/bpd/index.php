...
<div class="flex items-center justify-between mb-4">
    <div>
        ...
        <p class="text-xs text-slate-500 dark:text-slate-400">
            Kelola data anggota BPD beserta jabatan dan riwayat pendidikannya.
        </p>
    </div>
    <a href="<?= base_url('admin/bpd/create') ?>"
...
<div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden dark:bg-slate-900 dark:border-slate-800">
    <div class="p-3 border-b border-slate-100 dark:border-slate-800">
        <p class="text-xs text-slate-500 dark:text-slate-400">
            Daftar anggota BPD yang aktif beserta jabatan dan kontaknya.
        </p>
    </div>
    ...
    <div class="p-3 overflow-x-auto">
        <table id="tableBpd" class="min-w-full text-xs md:text-sm">
            <thead>
                <tr
...
<script>
    $(function() {
        ...
        // Ubah key storage agar tidak bentrok dengan perangkat desa
        const storageKey = 'bpd_page';

        // Selector diganti ke #tableBpd
        const table = $('#tableBpd').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                // Route URL ke admin/bpd/datatable
                url: baseUrl + 'admin/bpd/datatable',
                type: 'POST',
                data: function(d) {
                    d[csrfName] = csrfHash;
            ...
            columns: [{
                ...
                },
                {
                    ...
                    searchable: false,
                    render: function(row) {
                        // URL detail dan edit diarahkan ke /bpd/
                        const detailUrl = baseUrl + 'admin/bpd/detail/' + row.id;
                        const editUrl = baseUrl + 'admin/bpd/edit/' + row.id;
        ...
        $('#tableBpd').on('click', '.js-keep-page', function() {
        ...
        $('#tableBpd').on('click', '.btnDelete', function() {
            let id = $(this).data('id');
            ...
            Swal.fire({
                ...
                $.ajax({
                    // Route URL ke admin/bpd/delete
                    url: baseUrl + 'admin/bpd/delete',
                    type: 'POST',
                    dataType: 'json',
                    data: {
                        id: id,
                    ...
                    success: function(res) {
                        ...
                        if (res.status) {
                            Swal.fire({
                                icon: 'success',
                                title: 'Berhasil',
                                text: res.message || 'Data anggota BPD berhasil dihapus',
                                timer: 2000,
