...
<?= $this->section('title') ?>
<?= esc($pageTitle ?? 'Kategori Dokumen') ?>
<?= $this->endSection() ?>
...
<div class="w-full max-w-full">
    <div class="mb-4 flex items-center justify-between">
        <div>
            ...
            <p class="text-xs text-slate-500 dark:text-slate-400">
                Kelola kategori dokumen untuk website desa.
            </p>
        </div>
    ...
    <div class="rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-900">
        ...
        <div class="px-4 py-4 md:px-5 md:py-5 space-y-3">
            ...
            <form action="<?= base_url('admin/kategori-dokumen/save') ?>" method="post" class="space-y-3">
                <?= csrf_field() ?>
                ...
                <div>
                    ...
                    <input type="text" name="nama" id="nama"
                        ...
                        placeholder="Misal: Peraturan Desa" required>
                    <p class="mt-1 text-[11px] text-slate-500 dark:text-slate-400">
                        Slug akan dibuat otomatis dari nama (unik).
                    </p>
                </div>

                <div class="grid gap-3 md:grid-cols-2">
                    <div>
                        <label class="block text-xs font-medium text-slate-700 dark:text-slate-200 mb-1" for="urutan">
                            Urutan
                        </label>
                        <input type="number" name="urutan" id="urutan"
                            value="<?= esc($row['urutan'] ?? 0) ?>"
                            class="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs md:text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-primary-500/70 focus:border-primary-500 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100"
                            placeholder="0">
                    </div>

                    <div>
                        <label class="block text-xs font-medium text-slate-700 dark:text-slate-200 mb-1" for="is_active">
                            Status
                        </label>
                ...
                <div>
                    <label class="block text-xs font-medium text-slate-700 dark:text-slate-200 mb-1" for="deskripsi">
                        Deskripsi (opsional)
                    </label>
                    <textarea name="deskripsi" id="deskripsi" rows="4"
                        ...
                        placeholder="Keterangan singkat kategori..."><?= esc($row['deskripsi'] ?? '') ?></textarea>
                </div>
