...
<?= $this->section('title') ?>Galery<?= $this->endSection() ?>

<?= $this->section('content') ?>

<div class="flex items-center justify-between mb-4">
    <div>
        <h2 class="text-sm md:text-base font-semibold text-slate-800 dark:text-slate-100">Galery</h2>
        <p class="text-xs text-slate-500 dark:text-slate-400">Kelola foto galery website.</p>
    </div>

    <a href="<?= base_url('admin/galery/create') ?>"
...
<div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden dark:bg-slate-900 dark:border-slate-800">
    ...
    <div class="p-3 overflow-x-auto">
        <table id="tableGalery" class="min-w-full text-xs md:text-sm">
            <thead>
                <tr class="bg-slate-50 text-slate-600 border-b border-slate-100 dark:bg-slate-900/60 dark:text-slate-200 dark:border-slate-800">
                    ...
                    <th class="px-3 py-2 text-left font-medium whitespace-nowrap">Preview</th>
                    <th class="px-3 py-2 text-left font-medium whitespace-nowrap">Judul</th>
                    <th class="px-3 py-2 text-left font-medium whitespace-nowrap">Caption</th>
...
<script>
    $(function() {
        ...
        const table = $('#tableGalery').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: baseUrl + 'admin/galery/datatable',
                type: 'POST',
                data: function(d) {
                    d[csrfName] = csrfHash;
            ...
            columns: [{
                ...
                },
                {
                    ...
                    searchable: false,
                    render: (row) => {
                        const src = baseUrl + 'file/galery/' + (row.file_path ? row.file_path.split('/').pop() : '');
                        return `
            <a href="${src}" target="_blank" class="inline-flex items-center gap-2">
              <img src="${src}" class="w-12 h-12 rounded-xl object-cover border border-slate-200 dark:border-slate-700" alt="preview">
            </a>
          `;
                    },
                    className: 'px-3 py-2 whitespace-nowrap'
                ...
                {
                    data: 'caption',
                    render: (v) => v ? `<span class="text-slate-700 dark:text-slate-200">${v}</span>` : '-',
                    className: 'px-3 py-2'
                },
                {
                    data: 'is_active',
                ...
                },
                {
                    ...
                    searchable: false,
                    render: (row) => {
                        const editUrl = baseUrl + 'admin/galery/edit/' + row.id;
                        return `
        ...
        $('#tableGalery').on('click', '.btnDelete', function() {
            const id = $(this).data('id');

            Swal.fire({
                title: 'Hapus foto galery?',
                ...
                $.ajax({
                    url: baseUrl + 'admin/galery/delete',
                    type: 'POST',
                    dataType: 'json',
                    data: {
                        id,
