...
<div class="flex items-center justify-between mb-4">
    <div>
        ...
        <p class="text-xs text-slate-500 dark:text-slate-400">
            Kelola daftar agama yang digunakan pada data penduduk.
        </p>
    </div>
...
<div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden dark:bg-slate-900 dark:border-slate-800">
    <div class="p-3 border-b border-slate-100 dark:border-slate-800">
        <p class="text-xs text-slate-500 dark:text-slate-400">
            Sesuaikan nama, kode, dan urutan agama sesuai kebutuhan desa.
        </p>
    </div>
    ...
    <div class="p-3 overflow-x-auto">
        <table id="tableAgama" class="min-w-full text-xs md:text-sm">
            <thead>
                <tr
                    ...
                    <th class="px-3 py-2 text-left font-medium">Nama Agama</th>
...
<div id="modalAgama"
    ...
    <div
        ...
        <form id="formAgama" method="post"
            action="<?= base_url('admin/master-agama/save') ?>"
            class="px-4 py-3 space-y-3">
            <?= csrf_field() ?>
            <input type="hidden" name="id" id="agamaId">

            <div class="space-y-1.5">
                <label for="nama_agama"
                    class="text-xs font-medium text-slate-700 dark:text-slate-200">
                    Nama Agama <span class="text-rose-500">*</span>
                </label>
                <input type="text" name="nama_agama" id="nama_agama"
                    class="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm
                           focus:outline-none focus:ring-2 focus:ring-primary-500/60 focus:border-primary-500
                           dark:bg-slate-900 dark:border-slate-700 dark:text-slate-100"
                    placeholder="Contoh: Islam, Kristen, Katolik, Hindu, Buddha, Konghucu" required>
            </div>

            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div class="space-y-1.5">
                    <label for="kode_agama"
                        class="text-xs font-medium text-slate-700 dark:text-slate-200">
                        Kode Agama
                    </label>
                    <input type="text" name="kode_agama" id="kode_agama"
                        class="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm
                               focus:outline-none focus:ring-2 focus:ring-primary-500/60 focus:border-primary-500
                               dark:bg-slate-900 dark:border-slate-700 dark:text-slate-100"
...
<script>
    $(function() {
        ...
        const modal = document.getElementById('modalAgama');
        const modalTitle = document.getElementById('modalTitle');
        const idField = document.getElementById('agamaId');
        const namaField = document.getElementById('nama_agama');
        const kodeField = document.getElementById('kode_agama');
        const urutField = document.getElementById('urut');
        const aktifField = document.getElementById('is_active');

        // DataTables
        let table = $('#tableAgama').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: baseUrl + 'admin/master-agama/datatable',
                type: 'POST',
                data: function(d) {
                    d[csrfName] = csrfHash;
                        ...
                        return `
                            <div class="flex items-center gap-1.5">
                                <button type="button"
                                    ...
                                    data-id="${row.id}"
                                    data-nama="${row.nama_agama ?? ''}"
                                    data-kode="${row.kode_agama ?? ''}"
                                    data-urut="${row.urut ?? ''}"
        ...
        window.openAddModal = function() {
            modalTitle.textContent = 'Tambah Agama';
            idField.value = '';
            namaField.value = '';
            kodeField.value = '';
            urutField.value = '';
        ...
        function openEditModalFromRow(rowBtn) {
            const btn = rowBtn;
            modalTitle.textContent = 'Edit Agama';
        ...
        // Bind edit button
        $('#tableAgama').on('click', '.btnEdit', function() {
            openEditModalFromRow(this);
        });

        // Hapus dengan AJAX + Swal
        $('#tableAgama').on('click', '.btnDelete', function() {
            let id = $(this).data('id');
            ...
            Swal.fire({
                ...
                $.ajax({
                    url: baseUrl + 'admin/master-agama/delete',
                    type: 'POST',
                    dataType: 'json',
                    data: {
                        id: id,
                    ...
                    success: function(res) {
                        ...
                        if (res.status) {
                            Swal.fire({
                                icon: 'success',
                                title: 'Berhasil',
                                text: res.message || 'Data agama berhasil dihapus',
                                timer: 2000,
