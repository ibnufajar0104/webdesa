...
<div class="flex items-center justify-between mb-4">
    <div>
        ...
        <p class="text-xs text-slate-500 dark:text-slate-400">
            Kelola daftar jenis pekerjaan yang digunakan pada data penduduk.
        </p>
    </div>
...
<div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden dark:bg-slate-900 dark:border-slate-800">
    <div class="p-3 border-b border-slate-100 dark:border-slate-800">
        <p class="text-xs text-slate-500 dark:text-slate-400">
            Sesuaikan nama, kode, dan urutan pekerjaan sesuai kebutuhan desa.
        </p>
    </div>
    ...
    <div class="p-3 overflow-x-auto">
        <table id="tablePekerjaan" class="min-w-full text-xs md:text-sm">
            <thead>
                <tr
                    ...
                    <th class="px-3 py-2 text-left font-medium">Nama Pekerjaan</th>
...
<div id="modalPekerjaan"
    ...
    <div
        ...
        <form id="formPekerjaan" method="post"
            action="<?= base_url('admin/master-pekerjaan/save') ?>"
            ...
            <div class="space-y-1.5">
                ...
                <input type="text" name="nama_pekerjaan" id="nama_pekerjaan"
                    ...
                    placeholder="Contoh: Petani, Pedagang, PNS, Swasta" required>
            </div>

            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div class="space-y-1.5">
                    <label for="kode_pekerjaan"
                        class="text-xs font-medium text-slate-700 dark:text-slate-200">
                        Kode Pekerjaan
                    </label>
...
<script>
    $(function() {
        ...
        const modal = document.getElementById('modalPekerjaan');
        const modalTitle = document.getElementById('modalTitle');
        const idField = document.getElementById('pekerjaanId');
        const namaField = document.getElementById('nama_pekerjaan');
        const kodeField = document.getElementById('kode_pekerjaan');
        ...
        let table = $('#tablePekerjaan').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: baseUrl + 'admin/master-pekerjaan/datatable',
                type: 'POST',
                data: function(d) {
                    d[csrfName] = csrfHash;
        ...
        // Bind edit button
        $('#tablePekerjaan').on('click', '.btnEdit', function() {
            openEditModalFromRow(this);
        });

        // Hapus dengan AJAX + Swal
        $('#tablePekerjaan').on('click', '.btnDelete', function() {
            let id = $(this).data('id');
            ...
            Swal.fire({
                ...
                $.ajax({
                    url: baseUrl + 'admin/master-pekerjaan/delete',
                    type: 'POST',
                    dataType: 'json',
                    data: {
                        id: id,
                    ...
                    success: function(res) {
                        ...
                        if (res.status) {
                            Swal.fire({
                                icon: 'success',
                                title: 'Berhasil',
                                text: res.message || 'Data pekerjaan berhasil dihapus',
                                timer: 2000,
