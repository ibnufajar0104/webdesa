...
<div class="flex items-center justify-between mb-4">
    <div>
        ...
        <p class="text-xs text-slate-500 dark:text-slate-400">
            Kelola daftar jenjang pendidikan yang digunakan pada data penduduk.
        </p>
    </div>
...
<div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden dark:bg-slate-900 dark:border-slate-800">
    <div class="p-3 border-b border-slate-100 dark:border-slate-800">
        <p class="text-xs text-slate-500 dark:text-slate-400">
            Sesuaikan nama, kode, dan urutan pendidikan sesuai kebutuhan desa.
        </p>
    </div>
    ...
    <div class="p-3 overflow-x-auto">
        <table id="tablePendidikan" class="min-w-full text-xs md:text-sm">
            <thead>
                <tr
                    ...
                    <th class="px-3 py-2 text-left font-medium">Nama Pendidikan</th>
...
<div id="modalPendidikan"
    ...
    <div
        ...
        <form id="formPendidikan" method="post"
            action="<?= base_url('admin/master-pendidikan/save') ?>"
            class="px-4 py-3 space-y-3">
            <?= csrf_field() ?>
            <input type="hidden" name="id" id="pendidikanId">

            <div class="space-y-1.5">
                <label for="nama_pendidikan"
                ...
                <input type="text" name="nama_pendidikan" id="nama_pendidikan"
                    ...
                    placeholder="Contoh: SD/Sederajat, SMP, SMA, S1, S2" required>
            </div>

            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div class="space-y-1.5">
                    <label for="kode_pendidikan"
                        class="text-xs font-medium text-slate-700 dark:text-slate-200">
                        Kode Pendidikan
                    </label>
                    <input type="text" name="kode_pendidikan" id="kode_pendidikan"
                        class="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm
                               focus:outline-none focus:ring-2 focus:ring-primary-500/60 focus:border-primary-500
                               dark:bg-slate-900 dark:border-slate-700 dark:text-slate-100"
                        placeholder="Misal: 01, 02, 06">
                </div>
...
<script>
    $(function() {
        ...
        const idField = document.getElementById('pendidikanId');
        const namaField = document.getElementById('nama_pendidikan');
        const kodeField = document.getElementById('kode_pendidikan');
        const urutField = document.getElementById('urut');
        const aktifField = document.getElementById('is_active');

        // DataTables
        let table = $('#tablePendidikan').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: baseUrl + 'admin/master-pendidikan/datatable',
                type: 'POST',
                data: function(d) {
                    d[csrfName] = csrfHash;
                        ...
                        return `
                            <div class="flex items-center gap-1.5">
                                <button type="button"
                                    ...
                                    data-id="${row.id}"
                                    data-nama="${row.nama_pendidikan ?? ''}"
                                    data-kode="${row.kode_pendidikan ?? ''}"
                                    data-urut="${row.urut ?? ''}"
        ...
        // Bind edit button
        $('#tablePendidikan').on('click', '.btnEdit', function() {
            openEditModalFromRow(this);
        });

        // Hapus dengan AJAX + Swal
        $('#tablePendidikan').on('click', '.btnDelete', function() {
            let id = $(this).data('id');
            ...
            Swal.fire({
                ...
                $.ajax({
                    url: baseUrl + 'admin/master-pendidikan/delete',
                    type: 'POST',
                    dataType: 'json',
                    data: {
                        id: id,
                    ...
                    success: function(res) {
                        ...
                        if (res.status) {
                            Swal.fire({
                                icon: 'success',
                                title: 'Berhasil',
                                text: res.message || 'Data pendidikan berhasil dihapus',
                                timer: 2000,
