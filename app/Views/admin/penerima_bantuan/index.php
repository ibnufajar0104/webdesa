...
<?= $this->section('title') ?>Penerima Bantuan<?= $this->endSection() ?>

<?= $this->section('content') ?>

<div class="flex items-center justify-between mb-4">
    <div>
        <h2 class="text-sm md:text-base font-semibold text-slate-800 dark:text-slate-100">Penerima Bantuan</h2>
        <p class="text-xs text-slate-500 dark:text-slate-400">Kelola data penerima bantuan (relasi ke penduduk).</p>
    </div>
    <a href="<?= base_url('admin/penerima-bantuan/create') ?>"
...
<div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden dark:bg-slate-900 dark:border-slate-800">
    ...
    <div class="px-3 pt-3 pb-2 border-b border-slate-100 dark:border-slate-800">
        <div class="grid gap-2 sm:grid-cols-2 md:grid-cols-4">
            ...
            <div>
                <label class="block text-[11px] font-medium text-slate-600 dark:text-slate-300 mb-1">Bantuan</label>
                <select id="filterBantuan"
                    ...
                    <?php foreach (($bantuanList ?? []) as $b): ?>
                        <option value="<?= $b['id'] ?>"><?= esc($b['nama_bantuan']) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>

            <div>
                <label class="block text-[11px] font-medium text-slate-600 dark:text-slate-300 mb-1">Tahun</label>
                <input type="number" id="filterTahun" value="<?= (int)($tahunDefault ?? date('Y')) ?>"
                    class="w-full rounded-xl border border-slate-200 bg-white px-2.5 py-1.5 text-[11px] text-slate-800 focus:outline-none focus:ring-1 focus:ring-primary-500/70 focus:border-primary-500 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100">
            </div>
    ...
    <div class="p-3 overflow-x-auto">
        <table id="tablePB" class="min-w-full text-xs md:text-sm">
            <thead>
                <tr class="bg-slate-50 text-slate-600 border-b border-slate-100 dark:bg-slate-900/60 dark:text-slate-200 dark:border-slate-800">
                    ...
                    <th class="px-3 py-2 text-left font-medium">Bantuan</th>
                    <th class="px-3 py-2 text-left font-medium">Penduduk</th>
                    <th class="px-3 py-2 text-left font-medium whitespace-nowrap">Tahun</th>
                    <!-- <th class="px-3 py-2 text-left font-medium whitespace-nowrap">Nominal</th> -->
...
<script>
    $(function() {
        ...
        const table = $('#tablePB').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: baseUrl + 'admin/penerima-bantuan/datatable',
                type: 'POST',
                data: function(d) {
                    d[csrfName] = csrfHash;
                    d.filter_bantuan = $('#filterBantuan').val();
            ...
            columns: [{
                ...
                },
                {
                    data: null,
                    render: (row) => {
                        const nama = row.nama_penduduk || '-';
                        const nik = row.nik_penduduk || '-';
                        return `
            <div class="flex flex-col">
              <span class="text-xs text-slate-800 dark:text-slate-100">${nama}</span>
          ...
          `;
                ...
                },
                {
                    ...
                    render: (row) => {
                        const th = row.tahun || '-';
                        const pr = row.periode || '-';
                        return `<span class="whitespace-nowrap">${th}</span>`;
                    },
                    className: 'px-3 py-2 whitespace-nowrap'
                },
                // {
                //     data: 'nominal',
                //     render: (v) => v ? new Intl.NumberFormat('id-ID').format(v) : '-',
                //     className: 'px-3 py-2 whitespace-nowrap'
                // },
                {
                    data: 'status',
                    render: (v) => (parseInt(v) === 1) ?
                        '<span class="inline-flex px-2 py-0.5 rounded-full text-[11px] bg-emerald-50 text-emerald-700 border border-emerald-200 dark:bg-emerald-900/40 dark:text-emerald-200 dark:border-emerald-700">Diterima</span>' : '<span class="inline-flex px-2 py-0.5 rounded-full text-[11px] bg-slate-50 text-slate-700 border border-slate-200 dark:bg-slate-800/60 dark:text-slate-200 dark:border-slate-700">Nonaktif</span>',
                    className: 'px-3 py-2 whitespace-nowrap'
                },
                {
                    data: null,
                    ...
                    searchable: false,
                    render: (row) => {
                        const editUrl = baseUrl + 'admin/penerima-bantuan/edit/' + row.id;
                        return `
        ...
        $('#filterBantuan, #filterStatus').on('change', () => table.draw());
        $('#filterTahun').on('keyup change', () => table.draw());

        $('#btnResetFilter').on('click', function() {
            $('#filterBantuan').val('');
            $('#filterTahun').val('<?= (int)($tahunDefault ?? date('Y')) ?>');
            $('#filterStatus').val('1');
            table.draw();
        });

        $('#tablePB').on('click', '.btnDelete', function() {
            ...
            Swal.fire({
                ...
                $.ajax({
                    url: baseUrl + 'admin/penerima-bantuan/delete',
                    type: 'POST',
                    dataType: 'json',
                    data: {
                        id,
