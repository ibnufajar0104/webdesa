...
<div class="w-full max-w-full">
    <div class="mb-4 flex items-center justify-between">
        <div>
            ...
            <p class="text-xs text-slate-500 dark:text-slate-400">
                Form pengguna (role hanya superadmin/admin).
            </p>
        </div>
    ...
    <div class="rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-900">
        ...
        <div class="px-4 py-4 md:px-5 md:py-5 space-y-3">
            ...
            <form action="<?= base_url('admin/pengguna/save') ?>" method="post" class="space-y-4">
                <?= csrf_field() ?>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    <div class="md:col-span-2">
                        <label class="block text-xs font-medium text-slate-700 dark:text-slate-200 mb-1" for="nama">Nama</label>
                    ...
                    <div>
                        <label class="block text-xs font-medium text-slate-700 dark:text-slate-200 mb-1" for="username">Username</label>
                    ...
                    <div>
                        <label class="block text-xs font-medium text-slate-700 dark:text-slate-200 mb-1" for="email">Email (opsional)</label>
                    ...
                    <div>
                        <label class="block text-xs font-medium text-slate-700 dark:text-slate-200 mb-1" for="no_hp">No HP (opsional)</label>
                    ...
                    <div>
                        <label class="block text-xs font-medium text-slate-700 dark:text-slate-200 mb-1" for="role">Role</label>
                        <select name="role" id="role"
                            class="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs md:text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-primary-500/70 focus:border-primary-500 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100">
                            <?php foreach (($roleList ?? []) as $k => $v): ?>
                                <option value="<?= esc($k) ?>" <?= ($user['role'] ?? 'admin') === $k ? 'selected' : '' ?>>
                                    <?= esc($v) ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    ...
                    <div>
                        ...
                        <select name="is_active" id="is_active"
                            class="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs md:text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-primary-500/70 focus:border-primary-500 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100">
                            <option value="1" <?= (int)($user['is_active'] ?? 1) === 1 ? 'selected' : '' ?>>Aktif</option>
                            <option value="0" <?= (int)($user['is_active'] ?? 1) === 0 ? 'selected' : '' ?>>Non Aktif</option>
                        </select>
                    </div>

                    <div>
                        <label class="block text-xs font-medium text-slate-700 dark:text-slate-200 mb-1" for="password">
                            Password <?= ($mode ?? '') === 'edit' ? '(kosongkan jika tidak ganti)' : '' ?>
                        </label>
                        <input type="password" name="password" id="password"
                            class="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs md:text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-primary-500/70 focus:border-primary-500 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100"
                            <?= ($mode ?? '') === 'create' ? 'required' : '' ?>>
                        <p class="mt-1 text-[11px] text-slate-500 dark:text-slate-400">Minimal 6 karakter.</p>
                    </div>

                    <div>
                        <label class="block text-xs font-medium text-slate-700 dark:text-slate-200 mb-1" for="password_confirm">Konfirmasi Password</label>
